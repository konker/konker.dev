...
import * as unit from './cache-momento';
...
describe('middleware/cache-momento', () => {
  ...
  const stack = P.pipe(
    ...
    unit.middleware<In, common.Deps, any>(cacheKeyResolver, cache)
  );

  test('it should work as expected', async () => {
    // const result1 = await ruins.fromTaskEither(stack(TEST_IN)(deps));
    // const result2 = await ruins.fromTaskEither(stack(TEST_IN)(deps));
    const result1 = await P.pipe(
      stack(TEST_IN),
      P.Effect.provideService(Deps, {}),
