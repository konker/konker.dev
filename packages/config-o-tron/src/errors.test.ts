...
describe('errors', () => {
  ...
  describe('error factory functions', () => {
    ...
    describe('provider errors', () => {
      ...
      it('parameterNotFoundError should create correct error', () => {
        const error = parameterNotFoundError('db.password', '/config-o-tron/prod/api/db/password');

        expect(error.context.code).toBe(ErrorCode.PROV003);
        expect(error.context.path).toBe('db.password');
        expect(error.context.problem).toContain('/config-o-tron/prod/api/db/password');
      });
  ...
  describe('error classes', () => {
    it('ConfigOTronError should have correct exitCode', () => {
      const error = new ConfigOTronError({
        message: 'Test',
