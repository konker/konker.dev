    ...
    try {
      ...
      while (current !== root) {
        ...
        if (fs.existsSync(gitignorePath)) {
          ...
          if (content.includes('.config-o-tron/snapshots') || content.includes('.config-o-tron')) {
            return true;
          }
        }
...
): Effect.Effect<SnapshotSaveResult, ProviderError | SystemError | ValidationError | ConfigOTronError> {
  return pipe(
    ...
    Effect.flatMap(({ allServices, config, data, schemaHash, totalKeys }) => {
      ...
      const defaultPath = path.resolve(
        '.config-o-tron',
        'snapshots',
        `${options.service}-${config.env}-${timestamp}.json`
      );
      ...
      return pipe(
        ...
        Effect.flatMap((isIgnored) => {
          if (!isIgnored) {
            console.warn(
              '[config-o-tron] Warning: Snapshot directory may not be covered by .gitignore. ' +
                'Add ".config-o-tron/snapshots" to .gitignore to avoid committing secrets.'
            );
          }
...
): Effect.Effect<SnapshotRestoreResult, ProviderError | SystemError | ValidationError | ConfigOTronError> {
  return pipe(
      ...
      return pipe(
        ...
        Effect.flatMap(({ schemaHash }) => {
          ...
          if (snapshot.meta.schemaHash !== schemaHash) {
            console.warn(
              '[config-o-tron] Warning: Schema has changed since snapshot was created. Proceeding with --force-schema-mismatch.'
            );
          }
...
export function runSnapshotSave(
  options: SnapshotSaveOptions
): Effect.Effect<void, ProviderError | SystemError | ValidationError | ConfigOTronError> {
  return pipe(
...
export function runSnapshotRestore(
  options: SnapshotRestoreOptions
): Effect.Effect<boolean, ProviderError | SystemError | ValidationError | ConfigOTronError> {
  return pipe(
