...
): Effect.Effect<UpsertResult, ProviderError | ValidationError | SystemError | ConfigOTronError> {
  return pipe(
    // 1. Load schema
                ...
                return pipe(
                  ...
                  Effect.map((encType) => {
                    if (encType !== EncryptionType.SECURE_STRING) {
                      console.error(
                        `[config-o-tron] Warning: ${encryptionVerificationFailedError(canonicalKey).message}`
                      );
                      return false;
                    }
                    return true;
...
export function runUpsert(
  options: UpsertOptions
): Effect.Effect<void, ProviderError | ValidationError | SystemError | ConfigOTronError> {
  return pipe(
