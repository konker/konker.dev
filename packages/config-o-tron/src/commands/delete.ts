...
): Effect.Effect<DeleteResult, ProviderError | ValidationError | SystemError | ConfigOTronError> {
  return pipe(
      ...
      pipe(
        ...
        Effect.map((resolveResult) => {
          if (resolveResult._tag === 'Left') {
            ...
            console.warn(
              `[config-o-tron] Warning: Key '${options.key}' not found in schema. Proceeding with deletion anyway.`
            );
            return options.key;
          }
...
export function runDelete(
  options: DeleteOptions
): Effect.Effect<boolean, ProviderError | ValidationError | SystemError | ConfigOTronError> {
  return pipe(
