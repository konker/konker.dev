...
const RC_FILENAMES = ['config-o-tronrc.json5', 'config-o-tronrc.json'] as const;

/**
 * Load config-o-tronrc.json/config-o-tronrc.json5 from the current directory or parent directories
 */
function loadRcFile(startDir: string): Effect.Effect<ConfigOTronRcConfig | undefined, never> {
  return Effect.sync(() => {
...
function isCIMode(cliCi?: boolean): boolean {
  ...
  if (getEnvBool('CONFIG_O_TRON_CI')) return true;
...
export function resolveConfig(cliOptions: CLIOptions = {}): Effect.Effect<ResolvedConfig, never> {
  return pipe(
    ...
    Effect.map((rcConfig) => {
      ...
      const provider =
        cliOptions.provider ?? getEnvVar('CONFIG_O_TRON_PROVIDER') ?? rcConfig?.provider ?? DEFAULT_CONFIG.provider;

      const ssmPrefix =
        cliOptions.ssmPrefix ??
        getEnvVar('CONFIG_O_TRON_SSM_PREFIX') ??
        rcConfig?.ssmPrefix ??
        DEFAULT_CONFIG.ssmPrefix;

      const schema =
        cliOptions.schema ?? getEnvVar('CONFIG_O_TRON_SCHEMA') ?? rcConfig?.schema ?? DEFAULT_CONFIG.schema;

      const validation =
        cliOptions.validation ??
        (getEnvVar('CONFIG_O_TRON_VALIDATION') as ResolvedConfig['validation'] | undefined) ??
        rcConfig?.validation ??
      ...
      const format =
        ...
        (getEnvVar('CONFIG_O_TRON_FORMAT') as 'env' | 'json' | undefined) ??
        rcConfig?.format ??
