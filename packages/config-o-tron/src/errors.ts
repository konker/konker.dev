...
export function providerGuardMismatchError(provider: string, details: string): ProviderError {
  return new ProviderError({
    ...
    context: {
      ...
      remediation:
        'Verify providerGuards configuration, or set CONFIG_O_TRON_IGNORE_PROVIDER_GUARDS=1 for emergency override',
    },
  });
}
...
export function formatError(error: ConfigOTronErrorLike): string {
