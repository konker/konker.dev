...
describe('exportConfig', () => {
  ...
  it('loads config-o-tronrc.json when config overrides are omitted', async () => {
    tempDir = createTempDir('config-o-tron-api-test-');
    const { name } = registerMockProviderWithData(
      createBasicProviderData('api', {}, { prefix: '/custom', env: 'dev' })
    );

    writeJson(path.join(tempDir, 'config-o-tronrc.json'), {
      env: 'dev',
      provider: name,
      ssmPrefix: '/custom',
      schema: schemaBasicPath,
