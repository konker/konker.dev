# Specification: Config-o-tron Configuration & Secrets Management Tool

<!-- TOC -->

- [Specification: Config-o-tron Configuration & Secrets Management Tool](#specification-config-o-tron-configuration--secrets-management-tool)
  - [1. Goal](#1-goal)
  ...
  - [9. Implementation Requirements](#9-implementation-requirements)
    ...
    - [Config File (`config-o-tronrc.json` / `config-o-tronrc.json5`)](#config-file-config-o-tronrcjson--config-o-tronrcjson5)
...
Design and implement a CLI tool called **Config-o-tron** that orchestrates config providers and a pluggable validation layer (Effect Schema by default, Zod optional). It ensures that application configurations are securely retrieved, strictly validated, and safely exported for runtime use.
...
Config-o-tron operates on two related but distinct layers of configuration data:
...
Config-o-tron uses a **canonical key path** for CLI and internal operations:
...
By default, Config-o-tron does not print secret values in logs unless explicitly requested via an opt-in flag.
...
4. **Store:** Write to `.config-o-tron/snapshots/<service>-<env>-<timestamp>.json` with filesystem mode `0600`.
5. **Safety:** Warn if snapshots directory is under a git repo and not covered by `.gitignore`.
6. **Optional encryption:** If `--encrypt` is set, encrypt snapshot contents at rest using a key from `CONFIG_O_TRON_SNAPSHOT_KEY` or `--snapshot-key-file`.
...
type ProviderContext = {
  prefix: string; // e.g. "/config-o-tron"
...
Config-o-tron supports multiple schema validators behind a shared adapter interface. Effect Schema is the default; Zod is optional.

### Validator Selection

- Select via CLI `--validation <effect|zod>`, config `validation`, or env `CONFIG_O_TRON_VALIDATION`.
...
Adapters must normalize validator-specific errors into Config-o-tron's error catalog, including:
...
- Default `prefix` is `/config-o-tron`, configurable via `--ssm-prefix` or `CONFIG_O_TRON_SSM_PREFIX`.
- `<env>` should be sourced from `--env`, `CONFIG_O_TRON_ENV`, or `NODE_ENV`, with `dev` as a fallback.
...
- Config-o-tron uses canonical dot paths in the CLI (`database.url`) and providers translate dot paths to backend naming (`database/url` for SSM).

---

## 8. Multi-Source Composition

Config-o-tron may compose multiple SSM roots into a single config output.

### CLI Input

- `config-o-tron export <service> [--source <service>]...` to include additional services.
...
- Precedence order is: primary `<service>` first, then each `--source` in the order provided.
  - Example: `config-o-tron export api --source shared --source overrides`
    - SSM paths fetched (with `--env prod` and default prefix `/config-o-tron`):
      - `/config-o-tron/prod/api/database/url = postgres://api-main`
      - `/config-o-tron/prod/api/feature/enableBeta = false`
      - `/config-o-tron/prod/shared/database/url = postgres://shared`
      - `/config-o-tron/prod/shared/redis/url = redis://shared`
      - `/config-o-tron/prod/overrides/feature/enableBeta = true`
...
config-o-tron export <service> [options]
  ...
  --ssm-prefix <prefix>       # SSM path prefix (default: /config-o-tron)
  --schema <path>             # Schema path (default: src/schema.ts)
  --validation <effect|zod>   # Validation layer (default: effect)

# Upsert configuration value
config-o-tron upsert <service> <key> [value] [options]
  --provider <name>           # Provider name (default: aws-ssm)
  --env <environment>         # Environment name (overrides NODE_ENV)
...
config-o-tron validate [options]
...
config-o-tron list <service> [options]
  --provider <name>           # Provider name (default: aws-ssm)
  --env <environment>         # Environment name (overrides NODE_ENV)
...
config-o-tron list api --env prod

# List keys with values in JSON format (local dev only)
config-o-tron list api --env prod --format json --show-values

# Delete configuration value
config-o-tron delete <service> <key> [options]
  --provider <name>           # Provider name (default: aws-ssm)
  --env <environment>         # Environment name (overrides NODE_ENV)
...
config-o-tron snapshot save <service> [options]
  --source <service>          # Additional sources (repeatable)
  --provider <name>           # Provider name (default: aws-ssm)
  --env <environment>         # Environment name (overrides NODE_ENV)
  --output <path>             # Output path (default: .config-o-tron/snapshots/)
...
config-o-tron snapshot restore <snapshot-file> [options]
  --provider <name>           # Provider name (default: aws-ssm)
...
Config-o-tron must expose a TypeScript API for **export-only** usage. This API is intended for programmatic use (e.g., SST
...
```ts
import { exportConfig } from 'config-o-tron';
...
2. Environment variables (e.g., `CONFIG_O_TRON_ENV`, `CONFIG_O_TRON_SSM_PREFIX`)
3. Config file (`config-o-tronrc.json` or `config-o-tronrc.json5` in project root, if present)
4. Defaults (`NODE_ENV` for env, `/config-o-tron` for prefix, `dev` as fallback)

Supported environment variables:

- `CONFIG_O_TRON_ENV`: Default environment name
- `CONFIG_O_TRON_SSM_PREFIX`: Default SSM path prefix
- `CONFIG_O_TRON_PROVIDER`: Default provider name
- `CONFIG_O_TRON_SCHEMA`: Default schema path
- `CONFIG_O_TRON_VALIDATION`: Validation layer (`effect` or `zod`, default: `effect`)
- `CONFIG_O_TRON_CACHE`: Default provider fetch cache duration (e.g., `5m`, `0` to disable)
- `CONFIG_O_TRON_CI`: If set (`1`/`true`), disable prompts and require explicit confirmation flags
- `CONFIG_O_TRON_IGNORE_PROVIDER_GUARDS`: If set (`1`/`true`), skip provider guard checks (emergency use)
- `CONFIG_O_TRON_SNAPSHOT_KEY`: Snapshot encryption key (avoid setting in shared shells/CI logs)
- `NODE_ENV`: Fallback for environment name if `CONFIG_O_TRON_ENV` not set

### Config File (`config-o-tronrc.json` / `config-o-tronrc.json5`)

If present in the project root, `config-o-tronrc.json` or `config-o-tronrc.json5` provides non-secret defaults for Config-o-tron.
...
- `config-o-tronrc.json`/`config-o-tronrc.json5` must not contain secrets.
- CLI flags override environment variables, which override `config-o-tronrc.json`/`config-o-tronrc.json5`, which override defaults.
...
logic for evaluating them. Config-o-tron passes the provider-scoped guard config through as-is and does not interpret or
...
- Implementations should allow an explicit override for emergency use (e.g., a CLI flag or env var such as
  `CONFIG_O_TRON_IGNORE_PROVIDER_GUARDS=1`), but the default behavior should be to fail fast.
...
Config-o-tron parses provider strings and CLI/file inputs into typed values **before** validation. Parsing is explicit and schema-driven; no implicit type coercion.
...
When parsing `.env` files (for `validate`), Config-o-tron must map `.env` keys back to schema paths by generating the expected `.env` key for each schema leaf path (using the same output formatting rules) and matching case-sensitively. This avoids ambiguous “reverse snake_case” heuristics.
...
- **Non-interactive:** In `--ci` / `CONFIG_O_TRON_CI=1` mode (or when stdin is not a TTY), never prompt; require explicit flags like `--confirm`.
...
- **Snapshots:** Write snapshot files with mode `0600` and support optional encryption at rest (`--encrypt` using `CONFIG_O_TRON_SNAPSHOT_KEY` or `--snapshot-key-file`).
...
**Remediation:** Check command syntax, e.g., `config-o-tron export <service>`
...
- Verify write access for snapshot directory (`.config-o-tron/snapshots/`)
...
- Cache storage: Temp directory (`.config-o-tron/cache/`)
...
4. **Multi-region deployments:**
   - Run Config-o-tron in same region as SSM parameters to minimize latency
...
- `/config-o-tron/prod/api/feature/enableBeta = false`
- `/config-o-tron/prod/shared/redis/url = redis://shared`
- `/config-o-tron/prod/shared/api/timeoutMs = 60000`
- `/config-o-tron/prod/overrides/feature/enableBeta = true`
...
```
config-o-tron upsert api api.timeoutMs 6500 --env prod
```

Result: writes `/config-o-tron/prod/api/api/timeoutMs = 6500` (validated per the selected schema).

2. **Upsert invalid value**

```
config-o-tron upsert api database.url "not-a-url" --env prod
```
...
```
config-o-tron export api --source shared --source overrides --env prod --format json
```
...
config-o-tron validate --file .env.prod --schema src/schema.ts
```
...
config-o-tron list api --env prod --format table
```
...
config-o-tron delete api legacy.setting --env prod --confirm
```

Result: Deletes `/config-o-tron/prod/api/legacy/setting`, logs deletion with timestamp.

7. **Save configuration snapshot**

```bash
# Backup production config before major change
config-o-tron snapshot save api --source shared --env prod
```

Result: Saves to `.config-o-tron/snapshots/api-prod-2024-01-15T10-30-00.json` with metadata.

8. **Restore configuration from snapshot**

```bash
# Rollback to previous snapshot after failed deployment
config-o-tron snapshot restore .config-o-tron/snapshots/api-prod-2024-01-15T10-30-00.json --dry-run
```
...
echo "supersecret" | config-o-tron upsert api jwt.secret --stdin --env prod
```

Result: Writes `/config-o-tron/prod/api/jwt/secret` as SecureString, verifies encryption.
...
"Act as a Senior DevOps Engineer. Based on the Config-o-tron specification provided, generate a TypeScript implementation with the following requirements:
