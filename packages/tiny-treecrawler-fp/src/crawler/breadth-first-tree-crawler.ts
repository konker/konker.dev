...
import { FILES_FIRST, sortListingByFileType } from '../lib/utils.js';
import { notifyDirectoryEvent, notifyFileEvent } from './index.js';

export const BreadthFirstTreeCrawler: TreeCrawler = (
  tfs: TinyFileSystem,
  ...
  const crawlTree = (dirPath: string, rootPath: string, level: number): Effect.Effect<void, TinyTreeCrawlerError> => {
    return pipe(
      ...
      Effect.flatMap((_) => handlers[DIR](tfs, dirPath, level)),
      ...
      Effect.flatMap(sortListingByFileType(tfs, FILES_FIRST)),
