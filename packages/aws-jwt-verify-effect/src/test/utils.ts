/* eslint-disable fp/no-mutation,fp/no-unused-expression,fp/no-let,fp/no-nil */
/**
 * Utility functions used by unit and integration tests
 */

// ADAPTED FROM: https://github.com/awslabs/aws-jwt-verify/blob/main/tests/util/util.ts
...
export function generateKeyPair(
  ...
  jwk.alg = 'alg' in options ? options.alg : options.kty === 'RSA' ? 'RS256' : options.kty === 'EC' ? 'ES256' : 'EdDSA';
...
type JwtSignatureAlgorithm = keyof typeof JwtSignatureAlgorithmHashNames | 'EdDSA';
 ...
 * @param privateKey
 * @param options
 * @param options.produceValidSignature
 * @param options.addBogusPadding
 * @returns
 */
export function signJwt(
  header: { kid?: string | undefined; alg?: string; [key: string]: any },
  ...
  const signedJwt = [toSign, Buffer.from(signature).toString('base64url') + bogusPadding].join('.');
  return signedJwt;
}
