 ...
 * @param a - The Money instance to subtract from
 * @param b - The Money instance to subtract (must have same or compatible currency)
 * @returns A Result containing either the difference or an error
 * @function
 */
export function sub<S extends CurrencySymbol, S2 extends S>(a: Money<S>, b: Money<S2>): Effect.Effect<Money<S>, Error> {
  ...
  return pipe(
    NonNegativeRational.sub(a.rationalValue, b.rationalValue),
