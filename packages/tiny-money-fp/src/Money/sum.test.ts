...
import * as unit from './sum.js';

describe('Money', () => {
  describe('sum', () => {
    it('should sum three money values as expected with easy values', () => {
      const a = Effect.runSync(Money.fromNumber(111, 'EUR', 'cent'));
      const b = Effect.runSync(Money.fromNumber(222, 'EUR', 'cent'));
      const c = Effect.runSync(Money.fromNumber(333, 'EUR', 'cent'));
      const actual = Effect.runSync(unit.sum(a, b, c));

      expect(actual.rationalValue).toStrictEqual(NonNegativeRational(666 as NonNegativeInteger, 100 as Nat));
    });

    it('should sum three money values as expected with zero values', () => {
      const a = Effect.runSync(Money.fromNumber(0, 'EUR', 'cent'));
      const b = Effect.runSync(Money.fromNumber(0, 'EUR', 'cent'));
      const c = Effect.runSync(Money.fromNumber(0, 'EUR', 'cent'));
      const actual = Effect.runSync(unit.sum(a, b, c));
    ...
    it('should sum two money values as expected with tricky values', () => {
      /* Pathological case:
          > 0.01 + 0.06
          0.06999999999999999
        */
      ...
      const actual = Effect.runSync(unit.sum(a, b));

      expect(actual.rationalValue).toStrictEqual(NonNegativeRational(7 as NonNegativeInteger, 100 as Nat));
    });

    it('should sum two money values with different units as expected', () => {
      const a = Effect.runSync(Money.fromNumber(111, 'EUR', 'cent'));
    ...
    it('should not allow summing two money values with different currencies', () => {
      const a = Effect.runSync(Money.fromNumber(111, 'EUR', 'cent'));
      const b = Effect.runSync(Money.fromNumber(222, 'JPY', 'yen'));

      // This is a type test
      // @ts-expect-error currency mismatch expected
      const actual = unit.sum(a, b);
