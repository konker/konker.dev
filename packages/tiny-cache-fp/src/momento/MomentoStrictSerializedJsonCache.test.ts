...
describe('MomentoStrictSerializedCacheJson', () => {
  const cache: TinyCache<TestSchema, MomentoClientDeps> = StrictSerializedJsonCache(MomentoStringCache, TEST_SCHEMA);
  ...
  it('should be able to get a value which does not exist', async () => {
    const result = pipe(cache.getVal('non-existing-key'), Effect.provideService(MomentoClientDeps, deps));
    await expect(Effect.runPromise(result)).resolves.toStrictEqual(Option.none());
  });
