...
describe('aws-authorizer', () => {
  describe('transformLambdaAuthorizerArn', () => {
    it('should generate a response with the correct effect', () => {
      ...
      expect(provide(actual, undefined)._unsafeUnwrap()).toEqual(
        'arn:aws:execute-api:eu-north-1:898403669204:caidu80ct6/$default/*'
      );
    });
    ...
    it('should behave as expected with bad input', () => {
      ...
      expect(provide(actual, undefined)._unsafeUnwrapErr().message).toBe('Invalid arn');
    });
  });
