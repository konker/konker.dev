...
import { TEST_TOKEN, TEST_TOKEN_STRING_PAYLOAD } from '../test/fixtures/test-jwt-tokens.js';
...
describe('jwt/common', () => {
  ...
  describe('checkJwtPayloadIssSub', () => {
    ...
    it('should return an error if the token is invalid, missing issuer', () => {
      const actual = unit.checkJwtPayloadIssSub({ foo: 'bar', sub: TEST_JWT_SUB });
      expect(provide(actual, undefined)._unsafeUnwrapErr().message).toBe('Invalid token payload: missing iss or sub');
    });

    it('should return an error if the token is invalid, missing subject', () => {
      const actual = unit.checkJwtPayloadIssSub({ foo: 'bar', iss: TEST_JWT_ISS });
      expect(provide(actual, undefined)._unsafeUnwrapErr().message).toBe('Invalid token payload: missing iss or sub');
    });
    ...
    it('should return an error if the token is invalid, string payload', () => {
      ...
      expect(provide(actual, undefined)._unsafeUnwrapErr().message).toBe('Invalid token payload: string');
    });

    it('should return an error if the token is invalid, null', () => {
      const actual = unit.checkJwtPayloadIssSub(jwt.decode('banana'));
      expect(provide(actual, undefined)._unsafeUnwrapErr().message).toBe('Invalid token payload: null');
    });
  });
});
