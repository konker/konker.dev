import { provide } from '@konker.dev/neverthrow-r';
...
describe('jwt', () => {
  ...
  describe('jwtDecodeToken', () => {
    it('should decode a token', () => {
      ...
      expect(provide(actual, undefined)._unsafeUnwrap()).toStrictEqual(TEST_SIGNED_PAYLOAD);
    });

    it('should fail to decode a token with a string payload', () => {
      const actual = unit.jwtDecodeToken(TEST_TOKEN_STRING_PAYLOAD);
      expect(provide(actual, undefined)._unsafeUnwrapErr().message).toBe('Invalid token payload');
    });
  });
  ...
  describe('jwtSignToken', () => {
    it('should sign a token', () => {
      ...
      expect(provide(actual, undefined)._unsafeUnwrap()).toBe(TEST_TOKEN);
    });
  });
  ...
  describe('jwtVerifyToken', () => {
    it('should verify a valid token', () => {
      ...
      expect(provide(actual, undefined)._unsafeUnwrap()).toStrictEqual({
        userId: 'test-sub',
        verified: true,
        ...TEST_SIGNED_PAYLOAD,
      });
    ...
    it('should return verified false if the token is invalid, wrong secret', () => {
      const actual = unit.jwtVerifyToken(
        TEST_TOKEN,
        Object.assign({}, TEST_VERIFICATION_CONFIG, { signingSecret: 'wrong' })
      );
      expect(provide(actual, undefined)._unsafeUnwrap()).toStrictEqual({ verified: false });
    });

    it('should return verified false if the token is invalid, expired', () => {
      const actual = unit.jwtVerifyToken(TEST_TOKEN_EXPIRED, TEST_VERIFICATION_CONFIG);
      expect(provide(actual, undefined)._unsafeUnwrap()).toStrictEqual({ verified: false });
    });

    it('should return verified false if the token is invalid, wrong issuer', () => {
      const actual = unit.jwtVerifyToken(TEST_TOKEN_OTHER_ISSUER, TEST_VERIFICATION_CONFIG);
      expect(provide(actual, undefined)._unsafeUnwrap()).toStrictEqual({ verified: false });
    });

    it('should return verified false if the token is invalid, missing issuer', () => {
      const actual = unit.jwtVerifyToken(TEST_TOKEN_MISSING_ISSUER, TEST_VERIFICATION_CONFIG);
      expect(provide(actual, undefined)._unsafeUnwrap()).toStrictEqual({ verified: false });
    });

    it('should return verified false if the token is invalid, missing subject', () => {
      const actual = unit.jwtVerifyToken(TEST_TOKEN_MISSING_SUBJECT, TEST_VERIFICATION_CONFIG);
      expect(provide(actual, undefined)._unsafeUnwrap()).toStrictEqual({ verified: false });
    });

    it('should return verified false if the token is invalid, string payload', () => {
