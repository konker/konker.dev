...
describe('AwsSsmProvider', () => {
  ...
  it('should fetch and normalize parameters to dot paths', async () => {
    ssmMock.on(GetParametersByPathCommand).callsFake(async (input) => {
      if (!input.NextToken) {
        return {
          Parameters: [
            { Name: '/zenfig/prod/api/database/url', Value: 'postgres://db' },
            { Name: '/zenfig/prod/api/feature/enableBeta', Value: 'true' },
            { Value: 'missing-name' },
          ],
          NextToken: 'next',
        };
      ...
      return {
        Parameters: [
          { Name: '/zenfig/prod/api/database/timeout', Value: '30' },
          { Name: '/zenfig/prod/other/ignore', Value: 'nope' },
        ],
        NextToken: undefined,
      };
    });
  ...
  it('should upsert with SecureString parameters', async () => {
    ...
    expect(upsertCall?.args[0].input).toEqual({
      Name: '/zenfig/prod/api/database/url',
      Value: 'postgres://db',
      Type: 'SecureString',
      Overwrite: true,
    });
