...
export const middleware =
  ...
  <I extends Rec, O extends Rec, E, R>(
    wrapped: RequestResponseHandler<Override<I, WithValidatedPathParameters<V>>, R, O, E>
  ): RequestResponseHandler<I, R & WithLogger, O, E | MiddlewareError> =>
  (i: RequestW<I>) =>
    pipe(
      okAsyncR(i.pathParameters),
      tapLogger('debug', `[${TAG}] IN`),
      andThenParseAsyncR(schema, { message: 'Path parameters validation failed' }),
      mapErrAsync(toMiddlewareError),
      mapAsync((validatedPathParameters) =>
        makeRequestW(i, {
