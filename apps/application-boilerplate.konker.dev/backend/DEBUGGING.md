...
- HTTP requests to https://application-boilerplate.cloudfleet.konker.dev/ returning 503 Service Unavailable
...
```bash
export POD=$(kubectl get pods -n application-boilerplate -l app=application-boilerplatedotkonkerdotdev -o jsonpath='{.items[0].metadata.name}')
echo "Pod name: $POD"
```
...
```
Pod name: application-boilerplatedotkonkerdotdev-8b7b9c457-2w6ks
```

### 2. Check Service Endpoints

```bash
kubectl get endpoints -n application-boilerplate application-boilerplatedotkonkerdotdev
```
...
```
NAME                                 ENDPOINTS         AGE
application-boilerplatedotkonkerdotdev   10.42.0.41:3000   18h
```
...
```bash
kubectl logs -n application-boilerplate $POD
```
...
```bash
kubectl describe pod -n application-boilerplate $POD
```
...
kubectl run test-curl --image=curlimages/curl:latest --rm -it --restart=Never -- \
  curl -v http://application-boilerplatedotkonkerdotdev.application-boilerplate.svc.cluster.local:3000/
```

**Result:**

```
* Host application-boilerplatedotkonkerdotdev.application-boilerplate.svc.cluster.local:3000 was resolved.
* IPv4: 10.43.11.115
*   Trying 10.43.11.115:3000...
* Connected to application-boilerplatedotkonkerdotdev.application-boilerplate.svc.cluster.local (10.43.11.115:3000)
> GET / HTTP/1.1
> Host: application-boilerplatedotkonkerdotdev.application-boilerplate.svc.cluster.local:3000
...
< Content-Length: 93
<
{"apiId":"application-boilerplate-konker-dev","version":"0.0.2","ip":"UNKNOWN","konker":"RULEZ!"}
```
...
```bash
kubectl logs -n traefik traefik-87ddb4fc4-vbq2q --tail=100 | grep -i application-boilerplate
```

**Result:**

```
2025-12-05T19:01:55Z ERR error="kubernetes service not found: application-boilerplate/application-boilerplatedotkonkerdotdev"
  ingress=application-boilerplatedotkonkerdotdev namespace=application-boilerplate providerName=kubernetescrd
```
...
```bash
kubectl describe ingressroute -n application-boilerplate application-boilerplatedotkonkerdotdev
```
...
```bash
kubectl get service -n application-boilerplate application-boilerplatedotkonkerdotdev -o yaml
```
...
kubectl delete ingressroute -n application-boilerplate application-boilerplatedotkonkerdotdev
...
curl https://application-boilerplate.cloudfleet.konker.dev/
```

**Result:**

```json
{ "apiId": "application-boilerplate-konker-dev", "version": "0.0.2", "ip": "10.42.0.1", "konker": "RULEZ!" }
```
...
```bash
# Test with headers
curl -I https://application-boilerplate.cloudfleet.konker.dev/
```
